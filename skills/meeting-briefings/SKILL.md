...
If LinkedIn/profile lookup is unconfigured, ask whether to enable the user's authenticated LinkedIn or other profile source for future meeting briefings, unless the user explicitly asked for calendar-only prep. Save the preference in the config when the user answers. If the user only approves profile lookup for the current run, use it once but do not save it as the default.

The config should store durable preferences and source locations only: user identity aliases, calendar source, meeting filters, context source order, profile lookup status and scope, output destination, delivery-confirmation policy, and recurring schedule. Do not store cookies, tokens, profile contents, attendee research, message excerpts, or other harvested personal data in the config.
...
5. **Apply the profile quality gate.** Before writing the brief, check whether profile lookup is configured. If profile lookup is configured as enabled, use the saved profile source without asking again unless authentication fails or the requested action exceeds the saved scope. If profile lookup is unconfigured and the brief would rely only on calendar titles, event descriptions, or local files, pause and ask whether to enable LinkedIn/profile lookup for future meeting briefings. Offer a one-time-only option if the user does not want a saved default. Do not deliver a weak "profile not confirmed" brief unless the user declines profile lookup or no authenticated profile source is available. If identity is ambiguous or attendee background is thin, do not silently proceed when an authenticated profile source may resolve it. Ask to enable or use the profile source, or clearly state that the brief is calendar-only because the user declined or the source is unavailable.
